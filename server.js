...
app.post('/api/checklists', (req, res) => {          // motoristas não precisam de login
  const db = readDB();
  const r = { id: Date.now(), ...req.body };
  db.checklists.push(r);
  // alerta automático se houver NOK
  if (r.nok > 0) {
    db.alertas = db.alertas || [];
    db.alertas.push({
      id: Date.now()+1,
      tipo: 'nok',
      msg: `${r.nok} pendência(s) na viatura ${r.placa} — motorista ${r.motorista}`,
      checklist_id: r.id,
      data: r.data,
      lido: false
    });
...
app.get('/api/dashboard', requireAuth(['admin', 'chefe']), (req, res) => {
  ...
  const porViatura = db.viaturas.map(v => {
    const myCls = cls.filter(c => c.viatura_id == v.id);
  ...
  const motMap = {};
  cls.forEach(c => {
    if (!motMap[c.motorista]) motMap[c.motorista] = { nome: c.motorista, total: 0, nok: 0 };
    motMap[c.motorista].total++;
    if (c.status === 'nok') motMap[c.motorista].nok++;
  });
  const motoristas = Object.values(motMap).sort((a,b) => b.total-a.total);
  ...
  res.json({
    ...
    porViatura, porEquipe, porDia, motoristas
  });
});
...
app.get('/api/export/csv', requireAuth(), (req, res) => {
  ...
  const rows   = data.map(c => [c.id,c.data,c.placa,c.modelo,c.equipe,c.motorista,c.cpf||'',c.km||'',c.status,c.nok||0,(c.obs||'').replace(/,/g,';')]);
